...
def add_member(n,id1):
	...
	try:
			...
			# Cropped faces are saved in human_faces folder
			# Unique name is given to each cropped face 
			      temp_path = str(pid)+"__"+ str(face_count) + '.jpg'
			      path='./human__faces1/' +temp_path
                              from os import listdir
			      from os.path import isfile, join
			      path_files = [f for f in listdir("./human__faces1/") if isfile(join("./human__faces1/", f))]
			      import os
			      for fl in path_files:
				   os.remove("./human__faces1/"+fl)
                              cv2.imwrite(path, gray)
			      upload_to_ftp(temp_path)
                              cv2.putText(gray, str(face_count), (50, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,0), 2)
			# Display cropped faces using imshow built-in function
			      cv2.imshow('Cropped', gray)
                           if cv2.waitKey(1) == 27 or face_count == 30: # 27 is the Esc Key
		         	break
                        capture.release()
	                # Close all windows
	                cv2.destroyAllWindows()
